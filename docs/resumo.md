...
Se preferes o passeio completo e comentado, salta já para o [guia.md](guia.md) — são 3500 linhas e explicam tudo por ordem. Este documento é a versão que se lê antes de uma reunião.
...
**`PageDefinition`** — o contrato de uma página: `meta`, uma navegação opcional, uma lista de módulos, um footer opcional. É o que o renderer consome, e o único formato que o `core` conhece. → [core.md](core.md)

**`PageResponse`** — o que uma origem **responde** sobre um caminho: `ok`, `notFound` ou `redirect`. É um envelope à volta do `PageDefinition`, não um campo dentro dele — um redirect não tem página nenhuma. Substituiu um `PageDefinition | undefined` que dizia três coisas ao mesmo tempo. → [providers.md](providers.md#o-provider-diz-o-status-não-só-o-conteúdo)

**`PageSource` / `SiteSource`** — as duas classes abstractas que um provider implementa. Recebem `path` e `locale`; não conhecem Next.js. → [core.md](core.md)

**`Provider`** — o bundle de um CMS: `page`, `site`, e um `preview` opcional. Escolhido pela variável `PROVIDER`. → [providers.md](providers.md)

**`ModuleRegistry`** — o alias de um bloco resolve para um componente. Não existe lista de módulos no renderer. → [renderer.md](renderer.md)

**`Foundation`** — o que a aplicação precisa para renderizar: `modules`, `page`, `site`. Nada de CMS, nada de Next.js. → [architecture.md](architecture.md#foundation)
...
→ **[fluxos.md](fluxos.md)** — os dez fluxos, ficheiro a ficheiro

## O que surpreende

Cinco decisões que parecem erros até se ler o porquê.

**Um 404 responde 200.** Com duas raízes de route group, o `notFound()` do Next serve um shell vazio — medido. Renderizar o 404 como página normal serve HTML completo. A troca está justificada e medida em [routing.md](routing.md#o-404-é-conteúdo).

**Não há comentários no código.** O raciocínio vive aqui, nestes documentos, e não espalhado por docblocks que ninguém actualiza. As duas excepções são as âncoras `// plop:` — [conventions.md](conventions.md#comentários).
...
**O `mapApiPage` está por escrever.** De propósito: o formato é de quem desenhou a API, e um mapper genérico seria um palpite que parece funcionar — [api.md](api.md).

## Como fazer as coisas mais comuns

| Quero…                          | Faço                                                                                |
| ------------------------------- | ----------------------------------------------------------------------------------- |
| um módulo novo                  | `pnpm generate` — escreve os sete ficheiros e regista-o. → [modules.md](modules.md) |
| correr sem base de dados        | `PROVIDER=mock pnpm dev` → [providers.md](providers.md#o-provider-mock)             |
| mudar a página de 404           | marca uma página com `is404` no admin → [payload.md](payload.md#ishome-e-is404)     |
| um redirect                     | uma linha na collection `Redirects` → [payload.md](payload.md#redirects)            |
| ligar uma API em vez do Payload | escreve o `mapApiPage` → [api.md](api.md)                                           |
| tirar o Payload do projecto     | a receita está em [providers.md](providers.md#remover-o-payload)                    |
...
| Documento                          | Para quê                                        |
| ---------------------------------- | ----------------------------------------------- |
| [fluxos.md](fluxos.md)             | por onde passa cada pedido, ficheiro a ficheiro |
| [guia.md](guia.md)                 | o passeio completo, peça a peça                 |
| [architecture.md](architecture.md) | camadas, dependências e princípios              |
| [core.md](core.md)                 | os contratos                                    |
| [providers.md](providers.md)       | como se liga uma origem de conteúdo             |
| [payload.md](payload.md)           | o CMS: collections, cache, preview              |
| [api.md](api.md)                   | o provider de API externa                       |
| [modules.md](modules.md)           | escrever e gerar módulos                        |
